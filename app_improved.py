...
from modules.content import (
    ...
    generate_comparison_script, generate_dynamic_comparisons,
)
...
mode = st.radio(
    "Formato do vídeo",
    ["Lista de itens (vários cards)", "Comparação (coruja apontando)", "Comparações Dinâmicas (tema + período)"],
    horizontal=False,
)
...
# ---------------------------------------------------------------------------
# MODO 2: Comparação estilo "coruja apontando" (CORRIGIDO)
# ---------------------------------------------------------------------------
    ...
    if st.button("📝 Gerar roteiro da comparação"):
        with st.spinner("Gerando roteiro..."):
            # FIX: Adicionando os argumentos que estavam faltando
...
# ---------------------------------------------------------------------------
# MODO 3: Comparações Dinâmicas (NOVO!)
# ---------------------------------------------------------------------------
else:
    st.header("🎯 Comparações Dinâmicas por Tema e Período")
    st.caption("O app vai gerar múltiplas comparações automaticamente baseado no tema e período escolhido!")
    ...
    num_comparisons = st.slider(
        "Quantas comparações quer gerar?",
        min_value=1,
        max_value=10,
        value=3,
        step=1,
    ...
    if st.button("🔍 Gerar ideias de comparação"):
        with st.spinner(f"Gerando {num_comparisons} ideias de comparação para {theme}..."):
            comparisons = generate_dynamic_comparisons(
                theme=theme,
        ...
        if comparisons:
            st.session_state.dynamic_comparisons = comparisons
            st.success(f"✅ Geradas {len(comparisons)} ideias!")

            # Exibir as ideias
            st.subheader("Ideias de Comparação Geradas:")
            for idx, comp in enumerate(comparisons, 1):
                with st.expander(f"📊 Comparação {idx}: {comp['item1']} vs {comp['item2']}"):
                    st.write(f"**Item 1:** {comp['item1']}")
                    st.write(f"**Item 2:** {comp['item2']}")
                    st.write(f"**Por que comparar:** {comp['reason']}")

            # Selecionar qual comparação fazer
            st.subheader("Selecione uma comparação para criar o vídeo:")
            selected_idx = st.selectbox(
                "Qual comparação?",
                range(len(comparisons)),
                format_func=lambda i: f"{comparisons[i]['item1']} vs {comparisons[i]['item2']}",
            )

            if st.button("📝 Gerar roteiro para essa comparação"):
                selected_comp = comparisons[selected_idx]
                with st.spinner("Gerando roteiro..."):
                    st.session_state.comparison_script = generate_comparison_script(
                        selected_comp['item1'],
                        selected_comp['item2'],
                ...
                st.success("Roteiro gerado! Role para baixo para editar.")

            # Se tem roteiro, mostrar interface de edição
            if "comparison_script" in st.session_state and st.session_state.comparison_script:
                st.header("2. Revise e edite as falas")
                d = st.session_state.comparison_script
                c1, c2 = st.columns(2)
                d["item1_title"] = c1.text_input("Título exibido - item 1", d["item1_title"])
                ...
                if st.button("🎥 Renderizar vídeo final"):
                    ...
                    owl_thinking_path = "output/images/owl_thinking.jpg"
                    progress.progress(0.32, text="Buscando imagem da coruja pensativa...")
                    fetch_image_for_item(
                        "cute cartoon owl professor top hat monocle thinking pose white background",
                        owl_thinking_path,
                    ...
                    if not (ok1 and ok2):
                        st.error("Não consegui achar imagem pra um dos dois itens.")
                    else:
                        labels = ["intro1", "intro2", "explain1", "explain2"]
                        texts = [d["intro1_text"], d["intro2_text"], d["explain1_text"], d["explain2_text"]]
                        audio_paths = {}
                        for i, (label, text) in enumerate(zip(labels, texts)):
                            progress.progress(0.35 + i * 0.12, text=f"Gerando voz: {label}")
                            path = f"output/audio/{label}_{selected_idx}.wav"
                            synthesize(text, path, language_id=language, voice=selected_voice)
                            audio_paths[f"{label}_audio"] = path
                        ...
                        progress.progress(0.9, text="Montando vídeo final...")
                        out_path = f"output/video_comparison_{selected_idx}.mp4"
                        build_comparison_video(video_data, owl_path, out_path, owl_thinking_path=owl_thinking_path)
                        progress.progress(1.0, text="Pronto!")
                        ...
                        st.video(out_path)
                        with open(out_path, "rb") as f:
                            st.download_button("⬇️ Baixar vídeo", f, file_name=f"video_comparison_{selected_idx}.mp4")
